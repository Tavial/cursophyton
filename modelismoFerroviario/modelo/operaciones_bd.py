...
def registro_producto (producto):
    ...
    mydb = conectar ()
    cursor = mydb.cursor ()
    tupla = (producto.referencia, producto.nombre, producto.tipo, producto.escala, producto.fabricante, producto.precio, producto.stock, producto.tecnologia, producto.descripcion,producto.imagen )
...
def obtener_producto(referencia):   
    ...
    elemento = clases.Producto(referencia = producto_obtenido[0], nombre = producto_obtenido[1], \
        ...
        precio = producto_obtenido[5], stock = producto_obtenido[6], tecnologia = producto_obtenido[7], \
        descripcion = producto_obtenido[8], imagen = producto_obtenido[9])
    return elemento
...
def guardar_cambios_producto (producto): 
    ...
    mydb = conectar ()
    cursor = mydb.cursor()
    tupla = (producto.nombre, producto.tipo, producto.escala, producto.fabricante, producto.precio, producto.stock, producto.tecnologia, producto.descripcion,producto.imagen, producto.referencia)
    cursor.execute (sql,tupla)
    mydb.commit()
    mydb.disconnect()
