...
class VentanaPrincipal(QMainWindow): # implementamos la clase VentanaPrincipal que
    ...
    def cargarImagen (self): 
        ...
        if archivo [0]:
            # en la variable archivo se recoge separado por una coma la ruta y el tipo de las imágenes. Escogemos el elemento 0 que es el que contiene la ruta
            img = str(archivo[0])
            pixmap = QPixmap(img)
            alto_label = self.v_alta.visor_imagen.height()
            pixmap_redim = pixmap.scaledToHeight(alto_label)
            self.v_alta.visor_imagen.setPixmap(pixmap_redim)
            self.image = img
    ...
    def verImagen (self,imagen_a_mostrar): # Permite visualizar la imagen del producto cuando se pulsa el icono del ojo
        pixmap = QPixmap(imagen_a_mostrar)
        alto_label = self.v_tabla.visor_imagen.height()
        pixmap_redim = pixmap.scaledToHeight(alto_label)
        self.v_tabla.visor_imagen.setPixmap(pixmap_redim)
    ...
    def cargarImagen_editar (self):
        ...
        if archivo [0]:
            ...
            img = str(archivo[0])
            pixmap = QPixmap(img)
            alto_label = self.v_edicion.visor_imagen.height()
            pixmap_redim = pixmap.scaledToHeight(alto_label)
            self.v_edicion.visor_imagen.setPixmap(pixmap_redim)
            self.image = img  
